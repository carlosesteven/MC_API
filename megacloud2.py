...
class Megacloud:
    ...
    async def _get_secret_key(self, s: int) -> bytes:
        script_url = f"{self.base_url}/js/player/a/v2/pro/embed-1.min.js"
        print(f"{script_url}?v={s}")

        self.script = await make_request(script_url, {}, {"v": s}, lambda i: i.text())
    ...
    async def extract(self) -> dict:
        ...
        for i in s:
            key = await self._get_secret_key(i)
            print(f"key: {key}\n")
